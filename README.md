https://github.com/Belgradski/web-larek-frontend
...
```
Model:
StateApp - хранит состояние приложения(каталог товаров, корзину, даннные заказа).
LarekApi - отвечает за взаимодействие с сервером.

View:
Page, Card, Basket, DeliveryForm, ContactForm, Modal - компоненты интерфейса.

Presenter: 
EventEmitter - главный координатор(слушает события от View, вызывает методы у Model, обновляет View).
Логика управления через обработчики событий.
...
## Описание событий:
```
items:changed - обновляет каталог товаров на странице при изменении данных о товарах.

card:add - отвечает за добавление товара в корзину.

card:select - открытие модального окна с подробной информацией о товаре при его выборе.

basket:open - открытие корзины с отображением всех добавленных позиций.

card:delete - удаление товара из корзины.

basket:order - начало оформления заказа из корзины.

order:submit - обрабатывает переход от формы доставки к форме контактныйх данных.

contacts:submit - обрабатывает финальный этап оформления заказа.
